...
export default function Dashboard({ accessKey, initialState }: { accessKey: string; initialState: MahjongState }) {
  ...
  const load = useCallback(async () => { const response = await fetch(`/api/state?key=${encodeURIComponent(accessKey)}`, { cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(body.error ?? "读取失败"); setState(body); }, [accessKey]);
  ...
  async function save(payload: Record<string, unknown>) {
    ...
    try { const response = await fetch(`/api/mutate?key=${encodeURIComponent(accessKey)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...payload, actor: actor.trim() }) }); const body = await response.json(); if (!response.ok) throw new Error(body.error ?? "保存失败"); await load(); setMessage("已保存并写入编辑历史"); }
  ...
  return <main className="app-shell">
    ...
    {message && <div className="notice" role="status">{message}</div>}{state.demo && <div className="demo-banner">目前展示示例数据；连接数据库后即可共同编辑。</div>}
    {tab === "board" && <>
